#/!bin/bash
# Script for WAF Installation on Nginx Docker
# to restart nginx after installation using docker exec -it <conatiner id> nginx -s reload
# !!! Run as root
...
# Download and Build ModSecurity-Nginx Connector (works)
...
./configure --with-compat --add-dynamic-module=/opt/ModSecurity-nginx && make modules && cp objs/ngx_http_modsecurity_module.so /etc/nginx/modules
nginx -t
# Load Module into nginx.conf (works)
cp /etc/nginx/nginx.conf ~/nginx.conf
sed -i -e '3iload_module modules/ngx_http_modsecurity_module.so;\' ~/nginx.conf
